...
e = espnow.ESPNow()
e.init()
peer = b'\x78\xe3\x6d\x09\xcd\x48'
...
e.add_peer(peer)
my_peer = [0,1]
next_Assistance = 2#my_peer[random.getrandbits(1)]

while True:
    host, msg = e.irecv()     # Available on ESP32 and ESP8266
    if msg.decode() =='1':             # msg == None if timeout in irecv()
        print(host, msg)
        e.send(peer,'t'+str(random.getrandbits(3)))
