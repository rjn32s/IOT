...
void setup()
{
  Serial.begin(9600); // Initialize the baudrate to 9600 
...
void loop()
{
   x = random(0, 9+1);// Initialize a random number between 0 to 9
   onPeriod =blinkPeriod * ( x)/9;
   offPeriod = blinkPeriod - onPeriod;
   Serial.println("I am Here");  
   if (Serial.available()>0){
   
   N = random(3,10); // For random seletion of iternation
